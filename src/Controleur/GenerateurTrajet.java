...
public class GenerateurTrajet {
	...
	public static Trajet genererTrajet(Intersection a, Intersection b) {
		...
		// algorithme.shortestDistances[destination]);
	...
	/**
	 * Méthode récursive pour peupler les artères du trajet
	 * 
	 * @param destination
	 * @param parents
	 * @param trajet
	 ...
	 * @param arteres
	 */
	private static void genererTrajet(int destination, int[] parents, Trajet trajet, Vector<Intersection> intersections,
			Vector<Artere> arteres) {
		...
		if (source > Dijkstra.NO_PARENT) {
			...
			if (r != null) {
				trajet.ajouterProchainArtere(r);
			}
		}
	}
